...
/**
 *
 * This User Action assigns the current Owner to the "owner__c" field of a vSDK Product record to
 * the "Owner" role on each related vSDK Country Brand record.
 *
 * Create RecordRoleService object to set the "Owner" role for the current record:
 *
 * #1 Set the "owner__c" field of the vSDK Product to the current user and save
 * the record. #2 Create a map of vSDK Product ID to the new owner in the case that their are
 * multiple records. #3 On successful insert, query for all the related vSDK Country Brand records
 * that need to be updated and pass the them to changeOwnerRole(). #4 Retrieve the RecordRoles for
 * all vSDK Country Brand records with GetRecordRolesResponse #5 Iterate over the
 * GetRecordRolesResponse and retrieve the "owner__v" RecordRole for each record #6 Check if the new
 * owner is in the "owner__v" role with RecordRoleService.getUserInRecordRoles #7 Check if the Vault
 * Owner group is in the "owner__v" role with RecordRoleService.getGroupInRecordRoles #8 If the user
 * is not in the role or in the Vault Owner group, add the current user to the "owner__v" role for
 * each vSDK Country Brand record.
 *
 */
...
public class vSDKChangeOwnerUserAction implements RecordAction {
    public void execute(RecordActionContext recordActionContext) {
        ...
        List<String> productIds = VaultCollections.newList();
        ...
        String newOwner = requestContext.getInitiatingUserId();
        ...
        if (!productRecordList.isEmpty()) {
            RecordBatchSaveRequest saveRequest =
                recordService.newRecordBatchSaveRequestBuilder().withRecords(productRecordList).build();
            recordService.batchSaveRecords(saveRequest).rollbackOnErrors().execute();

            // Queries for related vSDK Country Brand records that need to be updated to a new owner
            // based on the vSDK Product Id. A list of Records is then built and sent to the
            // "changeOwnerRole()" function.
            //
            // The list of product IDs is supplied through a TokenRequest instead of being
            // concatenated into the query string, and the query is built with newQueryBuilder()
            ...
            queryService.query(queryRequest)
                .onSuccess(queryExecutionResponse -> {
                    queryExecutionResponse.streamResults().forEach(queryResult -> {
                        ...
                        country_brand =
                            recordService.readRecords(VaultCollections.asList(country_brand)).getRecords().get(id);
    ...
    // The "changeOwnerRole() function manages the RecordRoleService and modifies the owner roles
    // for each of the vSDK Country Brand records found in the query.
    private void changeOwnerRole(List<Record> recordList, Map<String, String> userIdMap) {
        ...
        // Add a check to verify is the specified Reviewer is in the Vault Owner group. This group
        // is defaulted on new records.
        Group vaultOwnerGroup =
            groupService.getGroupsByNames(VaultCollections.asList("vault_owners__v")).getGroupByName("vault_owners__v");

        // Get the "GetRecordRolesResponse" for the records being updated.
        // Then iterate over the list of records to get the RecordRole for the "owner__v" role and
        // see if the Reviewer is in that role.
        ...
        for (Record inputRecord : recordList) {
            ...
            boolean isUserInOwnerRole =
                recordRoleService.getUserInRecordRoles(userIdMap.get(productId), VaultCollections.asList(ownerRole))
                    .isUserInRecordRole(ownerRole);

            // Check if Vault Owner Group assigned to the record role
            boolean isGroupInOwnerRole =
                recordRoleService.getGroupInRecordRoles(vaultOwnerGroup, VaultCollections.asList(ownerRole))
                    .isGroupInRecordRole(ownerRole);

            // Create an RecordRoleUpdate to add the Reviewer user to Owner role if they aren't
            // already in the role and delete the existing owner.
        ...
        if (!recordRoleUpdateList.isEmpty()) {
            recordRoleService.batchUpdateRecordRoles(recordRoleUpdateList).rollbackOnErrors().execute();
        }
    }
